...
public class DbHandler {
    private final static String persistenceUnitName = "BankUnit";
    ...
    private static String formQueryString(HashMap<String, String> filters, Class<?> cl) throws NoSuchFieldException {
        ...
        for(String key : filters.keySet()) {
            ...
            queryFilters += "(";
            String compareOp = value.indexOf("~") >= 0 ? "<>" : "=";
            switch (type) {
                case NUMERIC:
                    queryFilters += "x." + key;
                    if(value.indexOf(">") >= 0) {
                        ...
                        queryFilters +=  compareOp + ":" + key;
                    }
                    break;
                case OBJ_ID:
                    String joinName = "x" + joinNum;
                    queryJoins += " JOIN x." + key + " " + joinName ;
                    queryFilters += joinName + ".id" + compareOp + ":" + key + "_id";
                    joinNum++;
                    break;
                default:
                    queryFilters += "x." + key;
    ...
    private static void setFilterParameters(HashMap<String, String> filters, Class<?> cl, Query query) throws NoSuchFieldException {
        for (Map.Entry<String, String> pair : filters.entrySet()) {
            ...
            String value = pair.getValue().replaceAll("~", "");
            switch (type) {
                case NUMERIC:
